...
import { InputWithValidate } from "./inputs";
import ErrorShow from "./errorShow";

const DynamicTwoFields = ({
  title,
  field1Name = "field1",
  field1Label,
  field1Placeholder,
  ...
  const [rows, setRows] = useState(
    value?.length ? value : [{ [field1Name]: "", [field2Name]: "" }]
  );

  // 🔑 Sync rows when RHF resets value
  useEffect(() => {
  ...
  }, [value, field1Name, field2Name]);

  // Keep RHF in sync when rows change
  useEffect(() => {
    onChange(rows);
  }, [rows]);
  ...
  const handleAdd = () =>
    setRows([...rows, { [field1Name]: "", [field2Name]: "" }]);
      ...
      {rows.map((row, index) => (
        ...
        >
          ...
          <div className="grid grid-cols-11 gap-4">
            ...
            {/* Add/Remove Buttons */}
            <div className="col-span-1 space-x-2 mt-7">
              {index === rows.length - 1 && (
                <button
                  type="button"
                  onClick={handleAdd}
...
export default DynamicTwoFields;
